...
if(isset($_POST['tambah'])){
	...
	$input = mysql_query("INSERT INTO tb_delegasi VALUES(NULL, '$nama_delegasi', '$nim', '$password', '$angkatan')") or die(mysql_error());
		...
		if($input){
				echo "<script>alert('Data Berhasi Ditambahkan !');window.location='../tb_delegasi.php';</script>";
			}else{
				echo "<script>alert('Data Gagal Ditambahkan !');window.location='../tb_delegasi.php';</script>";	
			}

	}else{
		echo "<script>alert('Tolong penuhi form pendaftaran!');window.location='../form_delegasi.php';</script>";
	}		
